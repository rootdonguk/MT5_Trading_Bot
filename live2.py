...
class PerfectGridBotWithManualControl:
    ...
    def connect_and_select_account(self):
        ...
        # 계좌 목록 가져오기 (mt5.account_info_list() 사용, 하지만 실제 MT5에서 여러 계좌 로그인 필요)
        # 여기서는 현재 로그인된 계좌만, 다중 계좌 지원 위해 가정
        ...
        print("\n연결된 계좌 목록 (현재 로그인된 계좌):")
        print(f"로그인: {account_info.login}")
        print(f"서버: {mt5.terminal_info().community_server if mt5.terminal_info() else 'Unknown'}")
        print(f"잔고: ${account_info.balance:,.2f}")
        print(f"자산: ${account_info.equity:,.2f}")
        
        # 다중 계좌 가정 (사용자가 여러 계좌 로그인 시 목록 출력)
        # 실제로 여러 계좌를 지원하려면 MT5 설정에서 여러 계좌 관리 필요
        answer = input("\n이 계좌 사용? (y/n): ")
        if answer.lower() != 'y':
            print("다른 계좌로 재로그인 해주세요.")
            mt5.shutdown()
            return False
